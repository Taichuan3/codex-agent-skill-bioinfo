...
- ClinicalTrials.gov 记录先 count、再限制 fields；记录 NCT ID、status、phase、intervention、condition、eligibility、primary outcome 和 results availability。
- trial existence/design 不等于 efficacy；有 results 也要区分 endpoint、arm、统计显著性、样本量和偏倚风险。
- PGx、FDA label、ClinVar、OpenFDA、Open Targets 和 cohort survival 证据回答不同问题；不要合并成治疗建议或个人风险结论。
- patient/cohort table 必须说明 inclusion/exclusion、population、follow-up、missingness、censoring 和 model covariates。
- 输出中明确 `Research use only` / `research-only` 边界；遇到个人医疗场景时拒绝具体建议，转为一般科研信息或建议咨询专业医生。
