...
Usage:
  python scripts/verify_agents_state_rules.py /path/to/AGENTS.md

This is not a canonical test suite. It checks that a patched project AGENTS.md
contains the hot/cold state-file rules and Directory Card routing rules that this
skill expects.
"""
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: verify_agents_state_rules.py /path/to/AGENTS.md")
        return 2
    p = Path(sys.argv[1])
    if not p.exists():
    ...
    checks = {
        "AGENTS remains under Hermes context cap": len(text) < 20000,
        ...
        "do not read all READMEs at startup": "不在 session start 读取所有目录 README" in text or "Do not read all directory READMEs at startup" in text,
        "README before scanning heavy artifact dirs": "README" in text and all(x in text for x in ["data/", "models/", "reports/"]),
        "README is navigation not final truth": "导航" in text or "navigation" in text.lower(),
        "nested AGENTS not result catalogs": "子目录 `AGENTS.md`" in text or "nested `AGENTS.md`" in text,
    }
    print(f"file={p}")
