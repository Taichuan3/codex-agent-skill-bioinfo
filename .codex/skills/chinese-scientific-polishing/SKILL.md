...
description: 用于中文科研文本润色、结构优化和可读性提升，包括摘要、引言、结果、讨论、图注、项目说明和审稿回复草稿。只做中文层面的润色，不做英文翻译；根据文稿部分控制数据密度、叙事功能和可读性。
---
...
- 重要结果需要解释清楚，但不需要把所有数据都写进正文；细节可放到图注、补充图或内部备注。
- 尽量使用直接、清楚、容易阅读的中文句式。避免复杂长句、过多术语堆叠和不必要的英文抽象名词。
- 不要把“审稿风险”“安全边界”“不能过度解释”等内部提醒写进正文。需要保留时，放入图注、备注或 `Tips`，不要打断结果段阅读。
...
7. 交付前自检：检查结论是否与证据相称、术语是否统一、章节功能是否正确、数据密度是否合适、是否仍有歧义或过强措辞。
