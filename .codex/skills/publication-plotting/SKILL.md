...
- 如果任务是在上一张图或同一 figure 的基础上继续修改，默认覆盖同一组最终输出文件，并更新对应 QC/source manifest；不要为每一轮微调自动另存新版本。只有用户明确要求保留草图、对比版本、历史版本或不同尺寸方案时，才使用带版本号或尺寸标记的新文件名。
- SVG/PDF 文字尽量保持可编辑。
- 不用图形暗示超出证据的机制或因果关系。
...
输出 figure contract、当前最终图文件路径、source data 路径、运行命令、QA note 和 caveat。若覆盖了上一版，明确说明“已覆盖同名最终输出”；若保留多个版本，说明保留原因。
