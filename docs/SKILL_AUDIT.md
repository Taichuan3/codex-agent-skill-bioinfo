...
- 未发现阻断发布的 P0 合规问题。
- 本轮只处理合规性、触发描述和包装说明；不新增科研功能 skill。
...
|---|---|---|---|---|---|---|---|
| `bioinfo-analysis-code` | Good：限定脚本、表格、统计、环境和代码整理 | Good：正文短，复杂规范放入 references | Good：已有 references | Good：强调输入、输出、环境、caveat | Good | Pass | 可在 v1.2 增加 Snakemake/Nextflow 收尾参考 |
...
| `research-project-planner` | Good：限定项目启动、背景调查、技术路线和可检验假设 | Good | Good：已有 references | Good：强调证据包和可复现计划 | Good | Pass | 可接入后续文献检索 skill |
...
| `submission-readiness-audit` | Good：限定投稿前或大版本收尾综合预检 | Good | Good：已有 references | Excellent：跨主文、图表、方法、数据、代码和引用 | Good | Pass | 可补 journal-specific checklist |
| `task-self-check` | Good：限定交付前轻量 QA | Good | Good：已有 references | Good：覆盖 evidence、file、code、figure、text | Good | Pass | 可补“轻量/深度 QA”分流表 |
